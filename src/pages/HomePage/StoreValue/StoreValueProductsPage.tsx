  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
      <div className="max-w-[1600px] mx-auto p-6 md:p-8 lg:p-10">
        ...
        {isLoading && (
          <div
            className={`grid ${viewMode === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:grid-cols-5" : "grid-cols-1"} gap-6`}
          >
            {[...Array(pageSize)].map((_, i) => (
              <div
                key={i}
                className="h-96 rounded-2xl bg-white dark:bg-slate-800/50 animate-pulse border border-slate-200 dark:border-slate-700"
              />
            ))}
          </div>
        )}
                      ...
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src =
                            "https://placehold.co/600x600/e2e8f0/94a3b8?text=Product";
                        }}
                      />
                  ...
                  <table className="w-full">
                    <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.id")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.image")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.title")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.sku")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.price")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.stock")}
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.status")}
                        </th>
                        <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">
                          {t("storeValue.columns.actions")}
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                      {filteredProducts.map((product) => (
                        <tr
                          key={product.id}
                          className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group ${selectedIds.includes(product.id) ? "bg-blue-50/50 dark:bg-blue-900/10" : ""}`}
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span
                                className={`font-bold ${product.offerPrice ? "text-xs text-slate-400 line-through" : "text-sm text-slate-900 dark:text-white"}`}
                              >
                                {product.price}
                              </span>
                              {product.offerPrice && (
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`text-sm font-bold ${product.stockQuantity < 10 ? "text-rose-500" : "text-slate-600 dark:text-slate-400"}`}
                            >
                              {product.stockQuantity}
                            </span>
                          </td>
            ...
            <div className="p-8">
              ...
              <h3 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-2">
                {productToRemove.length > 1
                  ? t("storeValue.remove.title")
                  : t("storeValue.remove.subtitle")}
              </h3>

              <p className="text-center text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                {productToRemove.length > 1
                  ? t("storeValue.remove.confirmBulk", {
                      count: productToRemove.length,
                    })
