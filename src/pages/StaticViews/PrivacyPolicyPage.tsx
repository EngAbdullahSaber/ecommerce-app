...
import { Shield, Save, RefreshCw, Globe, FileText } from "lucide-react";
...
export default function PrivacyPolicyPage() {
  ...
  const { data: viewResponse, isLoading, refetch } = useQuery({
    queryKey: ["static-view", "PRIVACY"],
    queryFn: () => GetSpecifiedMethod("/static-view/PRIVACY", lang),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(lang === "ar" ? "تم تحديث سياسة الخصوصية بنجاح" : "Privacy Policy updated successfully");
      refetch();
    },
    onError: () => {
      toast.error(lang === "ar" ? "فشل تحديث سياسة الخصوصية" : "Failed to update Privacy Policy");
    },
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30 dark:from-slate-900 dark:via-blue-900/20 dark:to-indigo-900/20 p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400 mt-1">
                {lang === "ar" ? "إدارة محتوى سياسة الخصوصية بمختلف اللغات" : "Manage privacy policy content in different languages"}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
            >
              <Save size={20} />
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-700">
          ...
          <div className="p-8">
            <div className="mb-6 flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/30">
            ...
            {isLoading ? (
              ...
              <div className="min-h-[400px] border border-slate-200 dark:border-slate-700 rounded-2xl focus-within:ring-2 ring-blue-500/20 transition-all overflow-hidden">
                <RichTextEditor
        ...
        <div className="p-6 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl border border-white/20 dark:border-slate-700/50">
          <div className="flex items-start gap-4 text-sm text-slate-600 dark:text-slate-400">
            ...
            <p>
              {lang === "ar" 
                ? "يتم عرض هذا المحتوى مباشرة للمستخدمين في قسم سياسة الخصوصية بالتطبيق. يرجى التأكد من دقة المعلومات القانونية قبل الحفظ."
                : "This content is displayed directly to users in the app's Privacy Policy section. Please ensure legal information is accurate before saving."}
            </p>
          </div>
        </div>
      </div>
