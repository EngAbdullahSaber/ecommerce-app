...
export const FileUploadComponent: React.FC<FileUploadProps> = ({
  ...
  const handleFileUpload = async (file: File): Promise<string> => {
    if (onUpload) {
      ...
      return typeof result === "string" ? result : URL.createObjectURL(file);
    }

    if (uploadEndpoint) {
      const formData = new FormData();
      formData.append("file", file); // Changed from "file" to "image" as it's common, or keep if unsure. Actually user didn't specify field name.
      ...
      // Assuming API returns { data: { url: "..." } } or similar based on common patterns in this project
      const data = response.data as any;
      return data.data?.url || data.url || data.path || data.imageUrl || (data.data && data.data[0]);
    }
